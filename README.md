...
- **Off-chain signing and authorization services.** Debits are initiated by off-chain card-network authorization logic, permissioned withdrawals and refund batches depend on off-chain services holding the withdrawal and refund ed25519 keys, and signed AutoDraw delegations are produced and held off-chain (the Killswitch is the holder's on-chain control over them).
- **Refund correctness lives off-chain.** `cardRefund` pays whatever batch the refund signer authorized: the contract verifies the operator, signature, nonce, and expiry, but nothing on-chain ties a batch to prior debits, checks that recipients are card holders, or prevents the same debit being refunded twice — that bookkeeping is the Partner's. The treasury float that pays batches is funded off-chain, and the single treasury nonce means batches must be minted and submitted strictly in sequence.
